defmodule Doppler.Gatherer do
  ...
  def handle_info(:kickoff, worker_count) do
    1..worker_count
    |> Enum.each(fn _ -> Doppler.WorkerSupervisor.add_worker() end)
  ...
  def handle_cast({:result, path, hash}, worker_count) do
    Doppler.Results.add_hash_for(path, hash)
    {:noreply, worker_count}
  end
  ...
  defp report_results() do
    ...
    Doppler.Results.find_duplicates()
